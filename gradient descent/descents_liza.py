...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        mse = (1 / x.shape[0]) * (residuals.T @ residuals)
        return mse
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        predictions = self.predict(x)
        gradient_1 = (2 / x.shape[0]) * x.T @ (predictions - y)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        gradient = (2 / x_batch.shape[0]) * x_batch.T @ (predictions - y_batch)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        beta_1_power = self.beta_1 ** self.iteration
        one_minus_beta_1_power = 1 - beta_1_power
        m_hat = self.m / one_minus_beta_1_power

        beta_2_power = self.beta_2 ** self.iteration
        one_minus_beta_2_power = 1 - beta_2_power
        v_hat = self.v / one_minus_beta_2_power

        step_size = self.lr()

        delta_w = step_size * m_hat / (np.sqrt(v_hat) + self.eps)
